            ...
            { data.map((element, i) =>
              <tr key={i} className="text text-desc -dark">
                  { this.props.columns && this.props.columns.map((column, i) => {
                    ...
                    if (key === "htag") {return <td key={i} className="text text-desc -dark"><span>{column.title}</span><a href={'/leaderboards?hashtag=' + element[key].strip} className="c-tag">{element[key]}</a></td>}
